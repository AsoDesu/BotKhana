...
class MoveIn extends BaseCommand {
	async execute(msg: Message, args: string[]) {
		...
		for (let [id, member] of msg.mentions.members) { // could be improved to allow for IDs or usernames instead of just mentions
			try {
				await member.voice.setChannel(member.voice.channel, `Moved to ${member.voice.channel.name} VC as requested by ${msg.author.username}`)
			} catch {
